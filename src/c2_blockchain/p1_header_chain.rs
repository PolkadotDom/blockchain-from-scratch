...
impl Header {
	...
	fn verify_sub_chain(&self, chain: &[Header]) -> bool {
		let mut parent = self;
		for header in chain {
			if hash(parent) != header.parent || header.height != parent.height + 1 {
				return false;
			}
			parent = header;
		}
...
// And finally a few functions to use the code we just wrote
...
fn build_valid_chain_length_5() -> Vec<Header> {
	let mut headers = vec![Header::genesis(); 5];
	for i in 1..5 {
		headers[i] = headers[i-1].child();
	}
	headers
}
...
fn build_an_invalid_chain() -> Vec<Header> {
	let mut headers = build_valid_chain_length_5();
	headers.push(headers[1].child());
	headers
}
